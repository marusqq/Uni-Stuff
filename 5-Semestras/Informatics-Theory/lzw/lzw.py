...
def get_m_number():
    ...
    m_length = intfrombits(bytestobits(file.read()[:1])) + 1
    #buffer = buffer[1:]
    ...
    for number in range(m_length):
        ...
        secret_number = int(intfrombits(bytestobits(file.read(1))))
...
def decompress(bytes):
    ...
    return decoder.decodefrombytes(bytes)
        ...
        if codepoint == CLEAR_CODE:
            ...
            if codepoint in self._codepoints:
                ...
                if None != self._prefix:
                    ...
                    m_number = get_m_number()

                    if (self.code_size() - END_OF_INFO_CODE) > abs(m_number):

                        if (m_number < 0):
                            debug(program = 'Decoder._decode_codepoint', text = 'DELETING DICTIONARIES', line = lineno())
                            self._clear_codes()

                        elif (m_number > 0):
                            debug(program = 'Decoder._decode_codepoint', text = 'DICTIONARIES HAVE BEEN FROZEN', line = lineno())
...
def bytestobits(bytesource):
    ...
    for b in bytesource:
        ...
        for bitplusone in range(8, 0, -1):
            ...
            debug(program = 'lzw.BYTEStoBITS', text = 'start,  = ' + str(nextbit), line = lineno())
            yield nextbit
